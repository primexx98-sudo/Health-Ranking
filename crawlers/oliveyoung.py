...
2026-07-13 검증 (자세한 내용은 설계서.md 참고):
- Playwright(진짜 브라우저) + playwright-stealth 조합도 여전히 막힘.
- stealth 제거 후 재테스트해도 동일하게 막혀 브라우저 지문 문제가 아님을 확인.
- 가정용 IP는 체크박스 없이 바로 통과하는 반면 Actions IP는 매번 Cloudflare Turnstile
  ("사람인지 확인하십시오") 인터랙티브 챌린지를 띄움 → Azure 데이터센터 IP 평판 자체가 원인.
  다이소·카카오는 동일 환경에서 문제없이 통과하므로 올리브영 랭킹 페이지(getBestList.do)에만
  더 강한 봇 방어가 걸려 있는 것으로 추정.
- Turnstile 체크박스를 Playwright로 직접 클릭 시도했으나 실패. frame.frames로 iframe은
  잡히지만 그 안의 body innerHTML이 항상 빈 문자열 — 실제 체크박스 UI는 일반 DOM이 아닌
  (열람 불가능한) 내부 렌더링 방식(shadow DOM 등)으로 그려져 Playwright 셀렉터로 접근 불가.
  즉 스크립트 자동 클릭으로는 우회 불가능 — Cloudflare가 의도적으로 막아놓은 지점.
- 결론: 코드 레벨 대응으로는 해결 불가. IP 레벨(주거용/모바일 프록시, self-hosted runner 등)
  대응이 필요하거나, 실패를 허용하고 스킵하는 현재 재시도+알림 정책을 유지해야 함.
"""
